...
def build(act="relu"):
    ...
    overview = [k.layers.Conv1D(8, i, activation="relu")(inp) for i in range(1, 9)]
    overview = [k.layers.GlobalMaxPooling1D()(x) for x in overview]
    overview = [k.layers.Reshape((1, 8))(x) for x in overview]

    info = tf.repeat(
        k.layers.Concatenate(axis=-1)([info, *overview]), K.shape(inp)[1], axis=1
    )
    ...
    for i in range(8):
        x = res_block(x, 512, 15, act=act)

    for i in range(2):
        x = res_block(x, 1024, 1, act=act)
